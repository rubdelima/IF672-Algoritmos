...
int binary_search(int array[], int start, int end, int object){
    ...
    if (array[mid] == object){
        ...
        }else if (start == end && array[mid] != object){
        return -1;
    }
}

int binary_search_v2(int array[], int start,int end, int key){
    if (start >=1){
        int mid = (start + end) / 2;
        if (key == array[mid]){
            return mid;
        } else if (key < array[mid]){
            return binary_search_v2(array, start, mid -1, key);
        }else{
            return binary_search_v2(array, mid +1, end, key);
    }}else{ return -1;}
}
// O v3 foi o mais rápido
